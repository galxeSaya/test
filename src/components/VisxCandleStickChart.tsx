...
const VisxCandleStickChart: React.FC<VisxCandleStickChartProps> = ({
  ...
  // 查找数据点对应的新闻点
  const getNewsPointForDate = (date: Date): CandleStickNewsPoint | undefined => {
  ...
  const handleMouseMove = (event: React.MouseEvent<SVGSVGElement>) => {
    ...
    const index = data.reduce((prev, curr, i) => {
      return Math.abs(+getDate(curr) - +x0) < Math.abs(+getDate(data[prev]) - +x0) ? i : prev;
    }, 0);
    ...
    if (newsPoint) {
      ...
      const distance = Math.sqrt(
        Math.pow(x - newsPointX, 2) + 
        Math.pow(y - newsPointY, 2)
      );
  ...
  return (
    <div style={{ position: 'relative' }}>
      <svg width={width} height={height} onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave}>
        <Group left={margin.left} top={margin.top}>
          ...
          {data.map((d, i) => {
            ...
            // 确定是否有关联的新闻点
            const hasNewsPoint = newsPoints.some(np => np.date.getTime() === d.date.getTime());
      ...
      {tooltipData && (
        ...
        >
          <div>
            <div><strong>日期:</strong> {tooltipData.candlePoint?.date.toLocaleDateString()}</div>
            <div><strong>开盘价:</strong> {tooltipData.candlePoint?.open.toFixed(2)}</div>
            <div><strong>收盘价:</strong> {tooltipData.candlePoint?.close.toFixed(2)}</div>
            <div><strong>最高价:</strong> {tooltipData.candlePoint?.high.toFixed(2)}</div>
            <div><strong>最低价:</strong> {tooltipData.candlePoint?.low.toFixed(2)}</div>
            <div><strong>成交量:</strong> {(tooltipData.candlePoint?.volume as number / 1000).toFixed(0)}K</div>
